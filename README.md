...
## Additional VAR Point-Estimation Pipeline

This repository now also includes a multivariate VAR(2) point-estimation workflow with 4 return series and three innovation families:

- Gaussian
- Student-t (iterative reweighting)
- Gaussian mixture (EM with shared coefficient matrix)

New entrypoints:

```bash
python -m src.run_var_point_estimation
```

New notebook:

- `notebooks/var_point_estimation_notebook.ipynb`

VAR outputs are organized as:
